...
import ora from "ora"
import { displayTokenInfo } from "./utils" // Import displayTokenInfo from utils
...
export async function fetchTokenData(contractAddress) {
  ...
  try {
    // Try pump.fun first
    const pumpResponse = await axios.get(`${PUMP_FUN_API}/coins/${contractAddress}`, {
      timeout: 10000,
    })
    ...
    try {
      ...
      const dexResponse = await axios.get(`${DEXSCREENER_API}/dex/tokens/${contractAddress}`, {
        timeout: 10000,
      })
      ...
      throw new Error("Token not found on pump.fun or DexScreener")
    }
  }
}
...
export async function watchToken(contractAddress) {
  ...
  try {
    ...
    console.error(chalk.red("Error fetching initial token data:"), error.message)
  }
...
function generateMockTrendingData(limit) {
  const mockTokens = [
    { name: "KODE", symbol: "KODE", price: 0.00123, marketCap: 1234567, volume24h: 89012 },
    { name: "DOGE", symbol: "DOGE", price: 0.08456, marketCap: 12345678, volume24h: 234567 },
    { name: "PEPE", symbol: "PEPE", price: 0.00000789, marketCap: 3456789, volume24h: 123456 },
    { name: "SHIB", symbol: "SHIB", price: 0.00001234, marketCap: 5678901, volume24h: 345678 },
    { name: "BONK", symbol: "BONK", price: 0.00002345, marketCap: 2345678, volume24h: 456789 },
  ]
